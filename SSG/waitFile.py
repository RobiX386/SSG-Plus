...
def waitFunc(eventFile):
    ...
    def changeStartingDriver():
        ...
        def writeData(x):
            ...
            startingDriverLabel.configure(text="Starting driver:\n"+x)
            with open(eventFile, "w") as config_file:
                event.write(config_file)
            changeWindow.destroy()

        for x in driverList:
            driver = ctk.CTkButton(selectDriverWrap, fg_color=buttonColor, text_color=textcolor, text=x, corner_radius=buttonRadius, command=lambda x=x :writeData(x))
